...
std::vector<std::string> Split(std::string whole, std::string separator) {
	...
	for (int i = 0; i < int(whole.length()); i++) {
		...
		if (i == 0 && (whole[i] == separator[0])){
			start = i+1;
		}
		else if (i == int(whole.length()) - 1) {
			// std::cout << "in else: start: " << start << ", length: " << length << std::endl;

			v.push_back(whole.substr(start, (length - start + 1)));
			start += i+1;
		}

		else if ((whole[i] == separator[0])) {
			// std::cout << "in else if: start: " << start << ", length: " << length << std::endl;

			v.push_back(whole.substr(start, (length - start)));
...
// 	std::vector<std::string>v = {"ban", "app"};
// 	Split("ban,app", ",");
